...
use crate::{get_session_with_cache, Format, CTX, TARGET_BLOCK_BYTESIZE, TARGET_BLOCK_SIZE};
...
async fn register_vortex_file(
    ...
    vortex_dir_segments[n_file_path_segments - 1] = if enable_compression {
        "vortex_compressed".to_string()
    } else {
        "vortex_uncompressed".to_string()
    };
        ...
        .is_err()
    {
        ...
        // println!(vtx_file.as_ref());

        // let f = OpenOptions::new()
        //     .write(true)
        //     .truncate(true)
        //     .create(true)
        //     .open(vtx_file.as_ref())
        //     .await?;
        let writer =
            ObjectStoreWriter::new(object_store.clone(), ObjectStorePath::from(vtx_file.path())).await?;

        VortexWriteOptions::default()
            .write(writer, data.into_array_stream())
            .await?;
    }
